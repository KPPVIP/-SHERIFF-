...
Citizen.CreateThread(function()
     while true do
          ...
          if IsControlJustPressed(1, 246) and alerteEnCours then
               if PlayerData.job ~= nil and PlayerData.job.name == 'sheriff' and AlertePrise == false then
                    TriggerServerEvent('PriseAppelServeur')
                    AlertePrise = true
                    TriggerEvent('TireEntenduBlips', coordsX, coordsY, coordsZ)
                    alerteEnCours = false
               else
                    ESX.ShowNotification('~r~Vous avez déjà un appelle en cours.')
               end
